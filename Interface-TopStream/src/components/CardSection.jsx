...
// Componente de seção que exibe um carrossel de cards com carregamento sob demanda (lazy loading)
const CardSection = ({ nomeSecao, fetchFunction, fetchParams = [], dados, tipo }) => {
    ...
    // Monitora a visibilidade da seção para buscar dados apenas quando o usuário rolar até ela
    useEffect(() => {
        if (dados) {
            setConteudo(dados);
    ...
    // Realiza o scroll horizontal suave do carrossel baseado na direção e largura do card
    ...
    // Verifica a posição de rolagem para mostrar ou ocultar os botões laterais
    const handleScroll = () => {
        if (!scrollRef.current) return;
    ...
    // Configura listeners de scroll e redimensionamento para atualizar os controles do carrossel
    useEffect(() => {
